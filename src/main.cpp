...
const string strMessageMagic = "SherlockCoinV2 Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // sherlockcoinv2: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
                        ...
                        bool* pfMissingInputs)
{
    ...
    // sherlockcoinv2: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
{
    int64_t nSubsidy = 0.000221 * COIN;
...
// sherlockcoinv2: find last block index up to pindex
...
static unsigned int GetNextTargetRequired_(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // Includes SherlockCoinV2 fix for wrong retargeting difficulty by Mammix2
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // sherlockcoinv2: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // sherlockcoinv2: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // sherlockcoinv2: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // sherlockcoinv2: track money supply and mint amount info
...
// sherlockcoinv2: total coin age spent in transaction, in the unit of coin-days.
...
// sherlockcoinv2: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProofOfStake)
{
    ...
    // sherlockcoinv2: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // sherlockcoinv2: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // sherlockcoinv2: record proof-of-stake hash value
    pindexNew->hashProofOfStake = hashProofOfStake;

    // sherlockcoinv2: compute stake modifier
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    if(pindexBest != NULL && pindexBest->nHeight > 1)
        nCoinbaseMaturity = 50; //coinbase maturity change to 50 blocks
    // Size limits
...
bool CBlock::AcceptBlock()
{
    ...
    // sherlockcoinv2: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // sherlockcoinv2: check proof-of-stake
    ...
    // sherlockcoinv2: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // sherlockcoinv2: getblocks may not obtain the ancestor block rejected
    ...
    // sherlockcoinv2: if responsible for sync-checkpoint send it
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "SherlockCoinV2", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        block.nVersion = 1;
        block.nTime    = 1401550836;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 1170712;
        if(fTestNet)
        {
            block.nNonce   = 1170712;
        }
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("f775035e39010bf5ba0355e02f807a05b4d4f937b115ffae5fca2075fdedcf90"));
        block.print();
        ...
        // sherlockcoinv2: initialize synchronized checkpoint
...
unsigned char pchMessageStart[4] = { 0x75, 0x40, 0x24, 0x10 };
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // sherlockcoinv2: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // sherlockcoinv2: tell downloading node about the latest block if it's
